...
function stackHasDockerfile(stack: string): boolean {
  return stack === 'nestjs' || stack === 'nodejs';
}

function dockerEntrypointForStack(stack: string): string {
  return stack === 'nodejs' ? 'dist/index.js' : 'dist/main.js';
}

function buildDockerCompose(options: {
  includeBackend: boolean;
  includeFrontend: boolean;
}): string {
  const services = ['version: "3.9"', 'services:'];
  ...
  if (options.includeFrontend) {
    ...
    if (options.includeBackend) {
      services.push('    depends_on:', '      - backend');
    }
  }
...
function buildServiceSubdirFiles(
  ...
  if (stack === 'nestjs') {
    ...
    files.push({
      path: `${dir}/Dockerfile`,
      content: buildDockerfile(nodeVersion, dockerEntrypointForStack(stack)),
    });
    ...
    ? [
        {
          ...
          content: buildDockerfile(
            nodeVersion,
            dockerEntrypointForStack(stack),
          ),
        },
...
function buildMicroservicesScaffold(
  ...
  const includeDocker =
    options.includeDocker ?? defaultIncludeDocker(backendStack);
  const backendHasDockerfile = stackHasDockerfile(backendStack);
  const frontendHasDockerfile = stackHasDockerfile(frontendStack);

  if (includeDocker && (backendHasDockerfile || frontendHasDockerfile)) {
    rootFiles.push({
      path: 'docker-compose.yml',
      content: buildDockerCompose({
        includeBackend: backendHasDockerfile,
        includeFrontend: frontendHasDockerfile,
      }),
    });
  }
...
export function defaultIncludeDocker(stack: string): boolean {
  ...
  return stackHasDockerfile(normalizedStack);
}
